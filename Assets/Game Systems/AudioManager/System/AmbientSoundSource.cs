...
public class AmbientSoundSource : MonoBehaviour
{
    ...
    // Use Start() to handle asynchronous loading after Awake is complete
    async void Start()
    {
        ...
        // If no clip in Inspector, try loading from Addressables
        else if (!string.IsNullOrEmpty(addressableKey))
        {
            // Start the asynchronous loading
            await LoadAddressableClipAsync(addressableKey);
            // If loading succeeded, _isClipReady is set to true inside the function
        }
        else
        {
    ...
    /// <summary>
    /// Loads the AudioClip asynchronously from Addressables.
    /// </summary>
    private async Task LoadAddressableClipAsync(string key)
    {
        if (_loadedAddressableClip != null) return;

        AudioClip clip = await AudioManager.LoadClipAsync(key);

        if (clip != null)
        {
            SetAudioCLip(clip);
            Debug.Log($"AmbientSoundSource got clip from manager: {key}");
        }
        else
        {
            Debug.LogError($"AmbientSoundSource: Failed to get clip from manager: {key}");
            _isClipReady = false;
        }
    }
