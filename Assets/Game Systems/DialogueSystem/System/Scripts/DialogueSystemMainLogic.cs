...
namespace DialogueSystem
{
    partial class DialogueSystemMain
    {
        ...
        private bool FindAllRequiredComponents()
        {
            ...
            if (_canvas == null)
            {
                return Error.Message($"{setupContext}: Can not find '_canvas' in children");
            }
        ...
        private bool Setup(DialogueSequence sequenceToPlay)
        {
            if (sequenceToPlay == null)
                return Error.Message("DialogueSystemMain->Setup: DialogueSequence sequenceToPlay == null");
            ...
            foreach (ActorSideOnScreen side in Enum.GetValues(typeof(ActorSideOnScreen)))
            {
                ...
                if (layerIndex == -1)
                {
                    return Error.Message($"Layer not found for side: {side.ToString()}.");
                }
        ...
        private bool AssignRenderTargetTextures()
        {
            foreach (ActorSideOnScreen side in Enum.GetValues(typeof(ActorSideOnScreen)))
            {
                if ((_screenModelRender[side.i()].texture = _textureRT[side.i()]) == null)
                    return Error.Message("DialogueSystemMain->Setup->AssignRenderTargetTextures: cannot assign texture render target to screen");
            }
            return true;
        }
        ...
        private bool PrepareActorModels()
        {
            ...
            foreach (var dialogueLine in _currentSequence.NodeMap.Values)
            {
                ...
                {
                    return Error.Message($"DialogueSystemMain->Setup->PrepareActorModels: cannot find actor model in '{dialogueLine._actor.name}'");
                }
