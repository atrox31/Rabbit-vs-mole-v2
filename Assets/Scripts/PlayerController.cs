...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("Animation")]
    [SerializeField] private float _animationSpeed = 1.8f;
    ...
    private readonly int StopWalkiHash = Animator.StringToHash("StopWalk");
    ...
    private void Awake()
    {
        ...
        _animationHashes ??= (Enum.GetValues(typeof(ActionType)) as IEnumerable<ActionType>)
            ...
            .Select(x => new KeyValuePair<ActionType, int>(x, Animator.StringToHash($"{_playerType}_{x}")))
    ...
    private void Update()
    {
        ...
        if (isMoving)
            _currentDirection = new Vector3(_moveAmount.x, 0, _moveAmount.y).normalized;
    }
    ...
    private void HandleOnInteract()
    {
        ...
        if (isHit && hit.IsInteractable(out var interactable))
            interactable?.Interact(_playerType, HandleAction, isBusy => IsBusy = isBusy);
    }
